...
This tutorial will guide you through the process of creating, resolving and updating DID using the example code provided in this folder.

## Environment
Example code is in JavaScript. You'll need to have node installed.
...
Firstly, you need to generate two keys that will be use in all operations regarding your DID: update key and recovery key.
In the code, you can generate secp256k1 or ed25519 private and public key pairs with a library from Transmute, but you can use your preferred library. The most important thing is that you keep  those two keyspairs secured, since they will be needed for all operations. They prove that your are the owner of the DID, and hence the owner of all credentials received for that DID.

Secondly, you need to create your DID document that conforms with W3C DID v1 specification. In there you will include all required and optional properties, as in this case we include the authentication keys with a service endopoint. For that reason, we also had to generate the authentication keypair (and keep it safe!!).

The resulting execution log shows the three generated private and public keys:
```
Your update key:
{
  publicJwk: {
...
Afterwards, we need to generate a 'create' body of the operation that will be submitted to sidetree node. We use the ion-sdk provided by the Decentralized Identity Foundation that help with that matter. The operation body looks like:
```
{
   "type":"create",
   "suffixData":{
...
Finally, the generated body is POSTed to the /operations method of the sidetree node, resulting:
```
{
...
```
Where you can see your DID that was written in Cardano blockchain:
`did:ada:EiDfLfmqWKLEgbALdWg2jj3m8LFbfFSwPdee6JvFE5VfTQ`

## Resolve your DID to a DID Document
...
DID resolution is a simple GET request to /identifiers/{did}. You can use the code provided in `resolve.js` or just run a simple curl from your bash terminal:
`curl https://mnwbfd1eg2.execute-api.us-east-1.amazonaws.com/cardano/identifiers/did:ada:EiDfLfmqWKLEgbALdWg2jj3m8LFbfFSwPdee6JvFE5VfTQ`
```
{
   "@context":"https://w3id.org/did-resolution/v1",
   "didDocument":{
...
```
Lovely, you got DID document with all your **public** identity information!!
